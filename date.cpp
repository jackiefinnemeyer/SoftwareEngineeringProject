...
// project phase 1 additions
bool Date::check_leap_date(int day, int month, int year){
...
bool Date::check_pal_date(string str){
    // input format is M/D/YYYY
    month = atoi(str.substr(0, str.find('/')).c_str());
    ...
    reverse_string = date_string;
    
    // reversing the string
    reverse(reverse_string.begin(), reverse_string.end());
    
    // checking if equal 
    if (date_string == reverse_string){
        return true;
    }
    else{
        return false;
...
string Date::get_date() {
    ...
    ostringstream d_str, y_str;
    d_str << day;
    y_str << year;
    string day_str = d_str.str();
    string year_str = y_str.str();
